...
export function ReaderTestimonials({ items }: ReaderTestimonialsProps) {
  if (!items || items.length === 0) return null;

  return (
    <section className="bg-aera-cream/35 py-16 border-t border-b border-aera-champagne/30 text-left relative overflow-hidden font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-xl mx-auto mb-12 space-y-2">
          <span className="inline-flex items-center gap-1.5 text-[10px] font-bold tracking-widest text-aera-accent uppercase">
          ...
          {items.slice(0, 3).map((item, idx) => (
            ...
            >
              <div className="space-y-4">
                ...
                <p className="text-xs italic text-aera-muted leading-relaxed relative">
              ...
              <div className="flex items-center gap-3 border-t border-aera-champagne/15 pt-4 mt-6">
                {item.avatar ? (
                  <div className="relative w-9 h-9 rounded-full overflow-hidden border border-aera-champagne/50">
                    <Image src={normalizeMediaUrl(item.avatar)} alt={item.avatarAlt || item.name} fill className="object-cover" />
                  </div>
                ) : (
                  <div className="w-9 h-9 rounded-full bg-aera-cream border border-aera-champagne/55 flex items-center justify-center">
                    <Quote size={12} className="text-aera-accent" />
                  </div>
                )}
                <div>
                  <h4 className="font-semibold text-xs text-aera-ink">{item.name}</h4>
                  {item.role && <span className="text-[10px] text-aera-muted">{item.role}</span>}
                </div>
              </div>
