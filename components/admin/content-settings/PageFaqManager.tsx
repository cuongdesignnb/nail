  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 text-left font-sans">
      ...
      <div className="lg:col-span-7 bg-white rounded-3xl p-6 border border-aera-champagne/45 shadow-luxury">
        ...
        {loading ? (
          <p className="text-xs text-aera-muted italic py-6 text-center">Loading FAQs...</p>
        ) : (
          <div className="space-y-4">
            {list.map((faq) => (
              <div key={faq.id} className="p-4 border border-aera-champagne/30 rounded-2xl flex justify-between items-start gap-4">
                <div className="space-y-1">
                  <h4 className="font-semibold text-xs text-aera-ink">{faq.question}</h4>
                  <p className="text-[10px] text-aera-muted leading-relaxed">{faq.answer}</p>
            ...
            {list.length === 0 && (
              <p className="text-xs text-aera-muted italic py-6 text-center">No FAQs found for this page.</p>
            )}
          </div>
        )}
      </div>
