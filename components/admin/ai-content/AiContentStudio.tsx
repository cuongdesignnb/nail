...
export function AiContentStudio() {
  ...
  if (!dashboard) return <div className="p-6 text-xs text-aera-muted">Loading AI Content Studio...</div>;

  const configured = dashboard.settings?.isEnabled && dashboard.settings?.apiKeyConfigured;

  return (
    <div className="space-y-6">
      {message && <div className="rounded-xl border border-aera-champagne/40 bg-aera-cream/30 px-4 py-3 text-xs text-aera-ink">{message}</div>}
      {!configured && (
      ...
      <section className="rounded-2xl border border-aera-champagne/35 bg-white p-5 space-y-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="font-heading text-base text-aera-ink flex items-center gap-2"><Sparkles size={18} className="text-aera-accent" /> Keyword Batch</h2>
            <p className="mt-1 text-xs text-aera-muted">Paste keywords line by line or comma separated. Duplicates are detected before queueing.</p>
          </div>
          <button type="button" onClick={load} className={`${buttonClass} border border-aera-champagne text-aera-ink`}><RefreshCw size={14} /> Refresh</button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <label className="flex items-center gap-2 rounded-xl border border-aera-champagne/50 px-3 py-2 text-xs font-semibold text-aera-ink">
            <input type="checkbox" checked={generateImages} onChange={(e) => setGenerateImages(e.target.checked)} />
            Generate cover images
          </label>
          <button type="button" onClick={createBatch} disabled={busy || !configured || rows.length === 0} className={`${buttonClass} justify-center bg-aera-accent text-white`}><Play size={14} /> Create Batch</button>
        </div>

        {generationMode === "schedule" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-xs font-semibold text-aera-ink flex items-center gap-1"><CalendarClock size={13} /> Start Time</label>
              <input type="datetime-local" value={scheduleStartAt} onChange={(e) => setScheduleStartAt(e.target.value)} className={inputClass} />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-semibold text-aera-ink">Interval Hours</label>
        ...
        {preview && (
          <div className="rounded-xl border border-aera-champagne/30 overflow-hidden">
            <div className="grid grid-cols-12 bg-aera-cream/40 px-3 py-2 text-[10px] font-bold uppercase text-aera-muted">
            ...
            {rows.map((row: any) => (
              <div key={row.normalizedKeyword} className="grid grid-cols-12 px-3 py-2 border-t border-aera-champagne/20 text-xs">
                <span className="col-span-5 text-aera-ink">{row.keyword}</span>
                <span className="col-span-3 text-aera-muted">{row.slug}</span>
      ...
      <section className="rounded-2xl border border-aera-champagne/35 bg-white overflow-hidden">
        <div className="px-5 py-4 border-b border-aera-champagne/30">
          <h2 className="font-heading text-base text-aera-ink flex items-center gap-2"><Bot size={18} className="text-aera-accent" /> Queue</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead className="bg-aera-cream/30 text-aera-muted uppercase text-[10px]">
            ...
            <tbody>
              {dashboard.jobs.map((job) => (
                <tr key={job.id} className="border-t border-aera-champagne/20">
                  <td className="px-4 py-3 font-semibold text-aera-ink">{job.keyword}<div className="text-[10px] font-normal text-aera-muted">{job.errorMessage}</div></td>
                  <td className="px-4 py-3"><span className="rounded-full bg-aera-cream px-2 py-1 text-[10px] font-bold text-aera-ink">{job.status}</span></td>
                  <td className="px-4 py-3 text-aera-muted">{job.textModel}</td>
                  <td className="px-4 py-3 text-aera-muted">{job.blogPostId ? "Created" : "-"}</td>
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-2">
                      <button type="button" onClick={() => jobAction(job.id, "run")} disabled={busy} className="text-aera-accent"><Play size={14} /></button>
              ...
              {dashboard.jobs.length === 0 && <tr><td colSpan={5} className="px-4 py-8 text-center text-aera-muted">No AI jobs yet.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
...
function Metric({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-2xl border border-aera-champagne/35 bg-white p-4">
      <div className="text-[10px] uppercase tracking-wide text-aera-muted font-bold">{label}</div>
      <div className="mt-1 text-xl font-bold text-aera-ink">{value ?? 0}</div>
    </div>
  );
}
