...
export default function SmtpSettingsForm() {
  ...
  async function save() {
    ...
    if (json.success) {
      ...
      toast.error("Unable to save SMTP settings.", json.error);
    }
  }

  async function testConnection() {
    const response = await fetch("/api/admin/settings/email/test-connection", { method: "POST" });
    const json = await response.json();
    if (json.success) {
      setSettings(json.data);
      toast.success("SMTP connection verified successfully.");
    } else {
      toast.error("Unable to connect to SMTP. Check host, port, encryption and credentials.", json.detail || json.error);
    }
  }
      ...
      <div className="space-y-5">
        <section className="rounded-2xl border border-[var(--admin-border)] bg-white p-5">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            ...
            <label className="text-sm font-semibold text-[var(--admin-ink)]">Encryption Mode<select className={inputClass} value={settings.secure ? "ssl" : "starttls"} onChange={(event) => update("secure", event.target.value === "ssl")}><option value="starttls">STARTTLS - Recommended for port 587</option><option value="ssl">SSL/TLS - Recommended for port 465</option><option value="none">None - Local development only</option></select></label>
            <Field label="SMTP Username" value={settings.username || ""} onChange={(value) => update("username", value)} />
            <Field label="SMTP Password" type="password" value={password} onChange={setPassword} placeholder={settings.hasPassword ? "Leave blank to keep current password" : "SMTP password"} />
          </div>
        </section>
        <section className="rounded-2xl border border-[var(--admin-border)] bg-white p-5">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <button type="button" onClick={save} disabled={saving} className="inline-flex h-10 items-center gap-2 rounded-[var(--admin-radius-md)] bg-[var(--admin-accent)] px-4 text-sm font-semibold text-white disabled:opacity-50"><Save size={15} /> {saving ? "Saving..." : "Save SMTP Settings"}</button>
            <button type="button" onClick={testConnection} className="inline-flex h-10 items-center gap-2 rounded-[var(--admin-radius-md)] border border-[var(--admin-border)] bg-white px-4 text-sm font-semibold"><ShieldCheck size={15} /> Test Connection</button>
          </div>
        </section>
...
function Field({ label, value, onChange, type = "text", placeholder }: { label: string; value: string; onChange: (value: string) => void; type?: string; placeholder?: string }) {
  return <label className="text-sm font-semibold text-[var(--admin-ink)]">{label}<input className={inputClass} type={type} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} /></label>;
}
