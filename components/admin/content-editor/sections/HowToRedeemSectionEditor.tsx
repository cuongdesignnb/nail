...
type HowToRedeemData = {
  title: string;
  steps: StepItem[];
};

type Props = { data: HowToRedeemData; onChange: (data: HowToRedeemData) => void };

export function HowToRedeemSectionEditor({ data, onChange }: Props) {
  function update(patch: Partial<HowToRedeemData>) {
    onChange({ ...data, ...patch });
  }
