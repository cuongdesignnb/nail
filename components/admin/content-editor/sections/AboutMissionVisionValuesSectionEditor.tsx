...
type Props = {
  data: IconCard[];
  onChange: (data: IconCard[]) => void;
};

export function AboutMissionVisionValuesSectionEditor({ data = [], onChange }: Props) {
  function update(newData: IconCard[]) {
    onChange(newData);
  }
  ...
  function addItem() {
    update([
      ...data,
      { id: crypto.randomUUID(), icon: 'gem', title: '', description: '' },
    ]);
  }
          ...
          <div key={item.id} className="bg-gray-50 rounded-lg p-4 space-y-3 border border-gray-200">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Describe this mission, vision, or value..."
